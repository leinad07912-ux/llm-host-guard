...
def verdict(rep: dict) -> str:
    n = {s: sum(1 for f in rep["findings"] if f["severity"] == s) for s in SEVERITIES}
    if n["CRITICAL"]:
        return f"🔴 {n['CRITICAL']} thing(s) to fix NOW" + (f", {n['HIGH']} this week" if n["HIGH"] else "")
    if n["HIGH"]:
        return f"🟠 {n['HIGH']} thing(s) to fix this week" + (f", {n['MED']} to tighten" if n["MED"] else "")
    if n["MED"]:
        return f"🟡 {n['MED']} thing(s) to tighten when convenient"
    return "🟢 Nothing urgent. Your LLM host looks closed to outsiders."
...
def print_text(rep: dict, color: bool = True) -> None:
    ...
    print(f"\n{verdict(rep)}   score {rep['score']}/10\n")
    if os.geteuid() != 0 and rep["os"] != "Windows":
        print("  (not root: firewall rules can't be read, so protected ports may still show as open — "
              "`sudo` gives the accurate score)\n")
    ...
    if any(f.get("fix_cmds") for f in rep["findings"]):
        print("\nnext: preview the fixes →  python3 -m llm_host_guard --fix --dry-run"
              "\n      apply them        →  sudo python3 -m llm_host_guard --fix")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--html", metavar="PATH", nargs="?", const="llm-host-guard-report.html",
                    help="write self-contained HTML report (default name if no PATH)")
    ap.add_argument("--open", action="store_true", help="open the HTML report in your browser (implies --html)")
